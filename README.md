...
→ `th:method="delete"`를 사용하면 `type="hidden"`을 갖는 `input` 태그를 자동으로 생성.
이 기능을 사용하기 위해서는 `application.properties`에 `spring.mvc.hiddenmethod.filter.enabled=true`로 설정하면 된다.
